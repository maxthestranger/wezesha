...
task("copy:css", function (done) {
  return src([paths.src.css + "/*.css", paths.src.css + "/*.css.map"]).pipe(
    dest(paths.dist.css)
  );
  done();
});
...
task("copy:js", function (done) {
  return src([paths.src.js + "/main.js", paths.src.js + "/main.js.map"]).pipe(
    dest(paths.dist.js)
  );
  done();
});
...
task("copy:html", function (done) {
  return src([paths.src.html, "!" + paths.src.base + "/docs/**/*"]).pipe(
